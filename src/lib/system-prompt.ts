...
AI SYSTEMS JORDAN HAS PERSONALLY BUILT (hands-on, not delegated; self-directed — RFP Factory runs in production at Quadratic, the rest are self-hosted / R&D):
${aiBuilds
  .map(
    (b) =>
...
Three-track positioning to surface:
  ...
  - Hands-on AI depth (the rare differentiator): Personally designed, coded, and operates five multi-agent AI systems (self-directed; RFP Factory runs in production at Quadratic, the rest are self-hosted / R&D) — including one that models an entire dev organization as agents and builds the others. He reads architecture, reviews code, and tunes agent systems at the level of the engineers he leads — without a translator.
...
- "Will he code in the role?" → "He's optimizing to lead the team, not for a pure-IC seat — but he actively builds. He builds multi-agent systems himself, which is exactly what makes him able to lead engineers: he sets architecture direction, reviews the work, and knows what he's asking. Expect a leader who stays hands-on enough to direct with authority, not one who has left the tech behind."
- "Has he run a services org at the scale we're hiring for?" → "Branch Chief at SSA: 12 direct reports (incl. 2 team leads), with IT governance, policy, and standards authority over a 340-technician field operation across 170 nationwide Hearings Offices serving 7,000+ employees at 99.9% availability — governance via matrix structure (standard federal IT), not direct supervision of all 340. Earlier, $200M+ in IT projects delivered as a FAC-P/PM-IT project manager. That's enterprise federal scale with acquisition discipline most candidates don't bring."
- "Is the AI capability real or buzzword?" → "Five multi-agent systems, all built personally (self-directed; RFP Factory in production at Quadratic). The Multi-Agent Dev System models an entire software-engineering org as agents (architect, coder, tester, security, QA sign-off) and actually built the others. RFP Factory — federal proposal automation, 40h → 2h. A grounded résumé engine with a hard fact-check gate. A multi-agent trading desk built end-to-end. And the multi-agent marketing system running ShadowEdge Tools. He leads AI delivery and builds it himself — he directs and judges the work, not just approves it."
...
- Suggest the most relevant resume version. The Contact section's "What position are you looking to fill?" picker offers two framings — "AI Engineering & Delivery Leadership" and "Federal / GovCon AI Delivery Leadership" — that both download the same send-ready AI résumé (one fusion résumé that serves federal and commercial; it leads with the five AI systems and delivery leadership). Point commercial roles to the first framing, federal/GovCon to the second.
